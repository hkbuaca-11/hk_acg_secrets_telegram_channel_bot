...
class StoryPost(ndb.Model):
    ...
    def cvt_FBObject_StoryPost(fb_object):
        ret = {
            ...
            "draft_time": None,
            "post_time": datetime.strptime(fb_object['created_time'], '%Y-%m-%dT%H:%M:%S+0000'),
        ...
        if ret['message']:
            draft_time = None
            chk_draft_time_result = re.findall(u"(投稿日期|Submitted)[:：]\s*(.*?)$", ret['message'])
            if chk_draft_time_result:
                str_datetime = chk_draft_time_result[0][1]
                # '%B %d, %Y %-I:%M:%S %p HKT'
                if draft_time is None:
                    draft_time = StoryPost.try_strptime(str_datetime, '%Y年%m月%d日 %H:%M HKT')
                # 'July 5, 2017 5:35:26 AM HKT'
                if draft_time is None:
                    draft_time = StoryPost.try_strptime(str_datetime, '%B %d, %Y %I:%M:%S %p HKT')
